...
import io.authme.sdk.collect.Lists;
import io.authme.sdk.util.Randoms;
...
public class LockPatternUtils {
     ...
     * @return The pattern.
     */
    public static List<io.authme.sdk.widget.LockPatternView.Cell> stringToPattern(String string) {
        List<io.authme.sdk.widget.LockPatternView.Cell> result = Lists.newArrayList();

        try {
            final byte[] bytes = string.getBytes(UTF8);
            for (int i = 0; i < bytes.length; i++) {
                byte b = bytes[i];
                result.add(io.authme.sdk.widget.LockPatternView.Cell.of(b / 3, b % 3));
            }
     ...
     */
    public static String patternToString(List<io.authme.sdk.widget.LockPatternView.Cell> pattern) {
        if (pattern == null) {
            return "";
        }
        ...
        for (int i = 0; i < patternSize; i++) {
            io.authme.sdk.widget.LockPatternView.Cell cell = pattern.get(i);
     ...
     */
    public static String patternToSha1(List<io.authme.sdk.widget.LockPatternView.Cell> pattern) {
        try {
     ...
     * @since v2.7 beta
     */
    public static ArrayList<io.authme.sdk.widget.LockPatternView.Cell> genCaptchaPattern(int size) throws IndexOutOfBoundsException {
        if (size <= 0 || size > io.authme.sdk.widget.LockPatternView.MATRIX_SIZE)
        ...
        int lastId = Randoms.randInt(io.authme.sdk.widget.LockPatternView.MATRIX_SIZE);
        usedIds.add(lastId);
        ...
        while (usedIds.size() < size) {
            ...
            final int lastRow = lastId / io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH;
            final int lastCol = lastId % io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH;
            ...
            final int maxDistance = Math.max(
                    Math.max(lastRow, io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH - lastRow),
                    Math.max(lastCol, io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH - lastCol));
            ...
            for (int distance = 1; distance <= maxDistance; distance++) {
                ...
                for (int line : lines) {
                    switch (line) {
                        case 0: {
                            if (rowA >= 0) {
                                randomValues = Randoms.randIntArray(Math.max(0, colA),
                                        Math.min(io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH, colC + 1));
                                for (int c : randomValues) {
                                    lastId = rowA * io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH + c;
                        ...
                        case 1: {
                            if (colC < io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH) {
                                randomValues = Randoms.randIntArray(Math.max(0, rowA + 1),
                                        Math.min(io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH, rowC + 1));
                                for (int r : randomValues) {
                                    lastId = r * io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH + colC;
                                    if (usedIds.contains(lastId))
                                        lastId = -1;
                                    else
                                        break;
                        ...
                        case 2: {
                            if (rowC < io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH) {
                                randomValues = Randoms.randIntArray(Math.max(0, colA),
                                        Math.min(io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH, colC));
                                for (int c : randomValues) {
                                    lastId = rowC * io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH + c;
                                    if (usedIds.contains(lastId))
                                        lastId = -1;
                                    else
                                        break;
                        ...
                        case 3: {
                            if (colA >= 0) {
                                randomValues = Randoms.randIntArray(Math.max(0, rowA + 1),
                                        Math.min(io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH, rowC));
                                for (int r : randomValues) {
                                    lastId = r * io.authme.sdk.widget.LockPatternView.MATRIX_WIDTH + colA;
                                    if (usedIds.contains(lastId))
                                        lastId = -1;
                                    else
                                        break;
        ...
        final ArrayList<io.authme.sdk.widget.LockPatternView.Cell> result = Lists.newArrayList();
        for (int id : usedIds) result.add(io.authme.sdk.widget.LockPatternView.Cell.of(id));
